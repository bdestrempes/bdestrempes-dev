...
The `featureFlags` service evaluates which users see the feature based on the configuration.
...
```typescript showLineNumbers
// ❌ Purely separated concerns at the component level...
...
// ✅ A more pragmatic approach with an aggregation layer...
class TeamsService {
...
In this article, we've explored different approaches to feature flags and some common implementation patterns. In [Part 2: Implementing Feature Flags at Scale](/articles/feature-flags-at-scale), we'll look at various ways to implement feature flags in larger applications.
